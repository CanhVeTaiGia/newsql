...
-- 2
delimiter &&
create procedure UpdateSalaryByID(EmployeeID int, inout CurrentSalary decimal(10,2))
begin
	update employees e
    set e.Salary =
    case
		when CurrentSalary < 20000000 then CurrentSalary * 1.1
		else CurrentSalary * 1.05
    end
    where EmployeeID = e.EmployeeID;
    select e.Salary into CurrentSalary
    from employees e
...
-- 3
delimiter &&
create procedure GetLoanAmountByCustomerID(CustomerID int, out TotalLoanAmount decimal(15,2))
begin
	select sum(l.LoanAmount) into TotalLoanAmount
    from customers c
    join loans l
...
-- 4
delimiter &&
create procedure DeleteAccountIfLowBalance (AccountID int)
begin
	if(select count(*) from accounts a where AccountID = a.AccountID) = 0
		then select "Tài khoản không tồn tại" as message;
    elseif(select balance from accounts a where AccountID = a.AccountID) < 1000000
		then delete from accounts a where AccountID = a.AccountID;
        select "Tài khoản đã được khóa thành công" as message;
	else
...
-- 5
delimiter &&
create procedure TransferMoney (AccountSend int, AccountReceive int, Amount decimal(15,2))
begin
    if not exists (select 1 from accounts a where a.AccountID = AccountSend) or not exists (select 1 from accounts a where a.AccountID = AccountReceive) 
		then select "Không tìm thấy tài khoản" as message;
	elseif ((select a.balance from accounts a where a.AccountID = AccountSend) < Amount )
		then select "Không đủ số dư trong tài khoản" as message;
	else
		update accounts a 
		set a.balance = a.balance - amount
        where a.AccountID = AccountSend;
        update accounts a
        set a.balance = a.balance + amount
        where a.AcountID = AccountReceive;
        select "Chuyển tiền thành công" as message;
    end if;
end &&
delimiter ;
-- 6
set @UpdatedSalary = 18000000;
call UpdateSalaryByID(4, @UpdatedSalary);
select @UpdatedSalary;
